...
function estimateTxSize(
  ...
  const opReturnOutputSize = 8 + varintSize(opReturnScriptLen) + opReturnScriptLen;
  return (
    TX_OVERHEAD +
...
async function buildOpReturnTx(params: {
  ...
  const { utxo, privateKey, scriptBytes } = params;
  const inputSats = Number(utxo.satoshis);
  const fee = calculateFee(estimateTxSize(1, scriptBytes.length));
