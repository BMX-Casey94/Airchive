 ...
 *
 * Keep in sync with `@airchive/types` `normaliseCallsign`.
 */
...
/** Best label for a fleet card title: real callsign, then registration. */
export function fleetTitle(
  callsign: string | null | undefined,
  icao: string | null | undefined,
  reg?: string | null,
): string {
  return normaliseCallsign(callsign, icao) ?? (reg?.trim() || "—");
}
